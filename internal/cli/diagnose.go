...
func newDiagnoseCommand(stdin io.Reader, stdout, stderr io.Writer) *cobra.Command {
	...
	command.RunE = func(command *cobra.Command, args []string) error {
		return runDiagnose(command.Context(), args, stdin, stdout, stderr)
	}
	return command
}

func runDiagnose(ctx context.Context, args []string, stdin io.Reader, stdout, stderr io.Writer) error {
