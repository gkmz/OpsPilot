...
- Provider / 模型：本地 `httptest` 假服务；未调用真实远程模型
- 对应 commit / tag：未提交工作区，提交后补充
...
go test ./...
go vet ./...
go test -run TestRunReadsSymptomFromArgument -v ./internal/app
```
...
- 本次没有证明什么：没有验证真实模型名、远程鉴权、诊断质量、多轮、流式、取消和会话持久化。
